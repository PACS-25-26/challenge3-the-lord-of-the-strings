...
The Doxygen configuration is generated by CMake. Once the build directory is configured, generate the documentation with:

```txt
make doc
```

The generated HTML entry point is `build/doc/html/index.html`. On systems with `xdg-open` or `open`, it can also be opened with:
